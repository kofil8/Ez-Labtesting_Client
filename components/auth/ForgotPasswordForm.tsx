  ...
  const handleAction = (formData: FormData) => {
    ...
    startTransition(async () => {
      try {
        ...
        if (res?.success) {
          setSubmitted(true);
          // Store email in sessionStorage for verify-otp flow
      ...
      <Card className='border-2'>
        ...
        <CardContent className='space-y-4'>
          ...
          <div className='flex justify-center'>
            <Link
              href={`/verify-otp?flow=reset&email=${encodeURIComponent(emailValue)}`}
            >
              <Button className='w-full'>Continue to Verify OTP</Button>
            </Link>
          </div>
        </CardContent>
