...
import { ShoppingCart, Clock, Beaker, Sparkles } from 'lucide-react'
...
export function EnhancedTestCard({ test, index = 0 }: EnhancedTestCardProps) {
  ...
  const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      hormone: 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300',
      ...
      thyroid: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300',
      cardiac: 'bg-pink-100 text-pink-700 dark:bg-pink-900 dark:text-pink-300',
      metabolic: 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300',
    }
  ...
  return (
    ...
    >
      <Card className="h-full awsmd-hover-lift border-2 border-gray-200 dark:border-gray-700 overflow-hidden relative group awsmd-rounded-xl">
        ...
        <div className={`absolute inset-0 ${getCategoryGradient(test.category)} opacity-0 group-hover:opacity-10 transition-opacity duration-500 blur-xl`} />
        
        {/* Top gradient accent */}
        <div className={`absolute top-0 left-0 right-0 h-1 ${getCategoryGradient(test.category)}`} />
        ...
        <CardContent className="p-6 relative">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
            <div>
              ...
              <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider">One-time payment</p>
            </div>
            
            <Button
              onClick={handleAddToCart}
              disabled={!test.enabled}
              className={`gap-2 ${getCategoryGradient(test.category)} text-white hover:scale-110 transition-all shadow-lg hover:shadow-xl awsmd-rounded font-bold px-6`}
            >
