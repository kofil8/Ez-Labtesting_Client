...
import "../../app/find-lab-center/mapbox-popup.css";
...
export function LabCenterMap({
  ...
  if (!isMounted) {
    return (
      <div className='w-full h-full relative flex items-center justify-center bg-gray-100 dark:bg-gray-800'>
  ...
  return (
    <div className='w-full h-full relative'>
      ...
      >
        {labCenters.map((lab) => (
          ...
          >
            ...
            >
              <MapPin
                className={`h-8 w-8 ${
                  hoveredLab === lab.id || selectedLab?.id === lab.id
                    ? "text-purple-600"
                    : "text-blue-600"
                }`}
                fill={
        ...
        {selectedLab && (
          <Popup
            longitude={selectedLab.location.lng}
            latitude={selectedLab.location.lat}
            anchor='top'
          ...
          >
            <div className='p-2 sm:p-3 max-w-[280px] sm:max-w-xs'>
              <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-2 sm:mb-3'>
                <h3 className='font-bold text-sm sm:text-base md:text-lg text-gray-900 dark:text-gray-100 pr-2 break-words'>
                  {selectedLab.name}
                </h3>
                {getDistance(selectedLab) !== null && (
                  <Badge
                    variant='outline'
                    className='bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800 font-semibold whitespace-nowrap text-xs sm:text-sm flex-shrink-0'
                  >
              ...
              <div className='flex flex-wrap items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3'>
                ...
                <Badge
                  variant={
                    selectedLab.status === "Open" ? "default" : "secondary"
                  }
              ...
              <div className='space-y-2 sm:space-y-2.5 text-xs sm:text-sm mb-2 sm:mb-3'>
                <div className='flex items-start gap-1.5 sm:gap-2 bg-gray-50 dark:bg-gray-800/50 rounded p-1.5 sm:p-2'>
                  ...
                  <span className='text-gray-700 dark:text-gray-300 leading-relaxed text-[11px] sm:text-sm break-words'>
                    {selectedLab.address}
                  </span>
                </div>
              ...
              <div className='pt-2 flex flex-col sm:flex-row gap-2 border-t border-gray-200 dark:border-gray-700'>
                {selectedLab.website && (
                  <Button
                    ...
                    variant='outline'
                    asChild
                    className='flex-1 border-2 text-xs sm:text-sm h-8 sm:h-9'
                  >
                    <a
