...
function getStatusMeta(status: string) {
  ...
  if (normalized === "COMPLETED") {
    return {
      label: "Results ready",
      className:
        "bg-emerald-50 text-emerald-700 border-emerald-200 dark:border-emerald-900/70 dark:bg-emerald-950/30 dark:text-emerald-200",
      icon: CheckCircle2,
    };
  }
  ...
  if (normalized === "FAILED" || normalized === "CANCELLED") {
    return {
      ...
      className:
        "bg-rose-50 text-rose-700 border-rose-200 dark:border-rose-900/70 dark:bg-rose-950/30 dark:text-rose-200",
      icon: AlertCircle,
    };
  }
  ...
  if (normalized === "REFUNDED") {
    return {
      label: "Refunded",
      className:
        "bg-slate-100 text-slate-700 border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200",
      icon: Receipt,
    };
  }
  ...
  if (normalized === "MANUAL_REVIEW") {
    return {
      label: "Needs review",
      className:
        "bg-amber-50 text-amber-700 border-amber-200 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-200",
      icon: AlertCircle,
    };
  }
  ...
  if (normalized === "LAB_ORDER_PLACED") {
    return {
      ...
      className:
        "bg-sky-50 text-sky-700 border-sky-200 dark:border-cyan-900/70 dark:bg-cyan-950/30 dark:text-cyan-200",
      icon: FlaskConical,
    };
  }
...
export function ResultsList() {
  ...
  if (!isAuthenticated || !user?.id) {
    return (
      <Card className='rounded-[28px] border-slate-200/80 bg-white/92 shadow-[0_20px_50px_-40px_rgba(15,23,42,0.35)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-black/30'>
        <CardContent className='pb-8 pt-8 text-center'>
          <AlertCircle className='mx-auto mb-3 h-10 w-10 text-slate-400' />
          <h2 className='text-xl font-semibold text-slate-900 dark:text-slate-100'>
            Sign in required
          </h2>
          <p className='mb-5 mt-2 text-slate-600 dark:text-slate-300'>
  ...
  if (orders.length === 0) {
    return (
      <Card className='rounded-[28px] border-sky-200 bg-sky-50/50 shadow-[0_20px_50px_-40px_rgba(15,23,42,0.35)] dark:border-cyan-900/70 dark:bg-cyan-950/20 dark:shadow-black/30'>
  ...
  const filterButton = (key: OrderFilter, label: string, count: number) => (
    <Button
      ...
      className={cn(
        "rounded-full",
        filter === key &&
          "bg-sky-600 hover:bg-sky-700 dark:bg-cyan-400 dark:text-slate-950 dark:hover:bg-cyan-300",
      )}
    >
      {label} ({count})
    </Button>
  ...
  return (
    <div className='space-y-5'>
      <Card className='rounded-[30px] border-slate-200/80 bg-[linear-gradient(135deg,rgba(14,165,233,0.12)_0%,rgba(255,255,255,0.96)_52%,rgba(16,185,129,0.08)_100%)] shadow-[0_24px_60px_-44px_rgba(15,23,42,0.35)] dark:border-slate-800 dark:bg-none dark:bg-slate-950 dark:shadow-black/30'>
        <CardContent className='grid gap-3 p-5 sm:grid-cols-2 xl:grid-cols-4'>
          <div className='rounded-[22px] border border-white/70 bg-white/85 p-4 dark:border-slate-800 dark:bg-slate-900'>
      ...
      {filteredOrders.length === 0 ? (
        <Card className='rounded-[28px] border-slate-200/80 bg-white/92 shadow-[0_20px_50px_-40px_rgba(15,23,42,0.35)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-black/30'>
          <CardContent className='pb-8 pt-8 text-center'>
            ...
            <Button
              ...
              className='mt-4 rounded-full dark:border-slate-700 dark:hover:bg-slate-900 dark:hover:text-cyan-300'
            >
              Show all orders
            </Button>
          </CardContent>
        ...
        <div className='space-y-5'>
          {filteredOrders.map((order) => {
            ...
            const isTrackable =
              tracking.status === "processing" || tracking.status === "pending";
              ...
              <div key={order.id} className='space-y-3'>
                <Card className='rounded-[28px] border-slate-200/80 bg-white/92 shadow-[0_20px_50px_-40px_rgba(15,23,42,0.35)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-black/30'>
                      ...
                      <div className='flex flex-wrap items-center gap-2'>
                        {isTrackable && (
                          <Button
                            asChild
                            size='sm'
                            className='rounded-full bg-gradient-to-r from-blue-600 to-cyan-500 text-white shadow-sm hover:from-blue-700 hover:to-cyan-600'
                          >
                            <Link
                  ...
                  <CardContent className='grid gap-3 sm:grid-cols-3'>
                    <div className='rounded-[20px] border border-slate-200/80 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-900'>
