...
function getStatusAppearance(status: OrderTrackingStatus["status"]) {
  ...
  if (status === "processing") {
    return {
      badge:
        "border-sky-200 bg-sky-50 text-sky-700 dark:border-cyan-900/70 dark:bg-cyan-950/30 dark:text-cyan-200",
      icon: Clock,
      iconTone: "text-sky-600",
      banner:
        "border-sky-200 bg-sky-50 text-sky-900 dark:border-cyan-900/70 dark:bg-cyan-950/30 dark:text-cyan-100",
    };
  }
...
export function OrderTrackingCard({
  ...
  return (
    <Card className='rounded-[28px] border-slate-200/80 bg-white/92 shadow-[0_20px_50px_-40px_rgba(15,23,42,0.35)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-black/30'>
      <CardHeader className='border-b border-slate-100/90 pb-4 dark:border-slate-800'>
        <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
          <div>
            <CardTitle className='flex flex-wrap items-center gap-2 text-lg text-slate-950 dark:text-slate-100'>
              <span>{orderNumber}</span>
              <Badge variant='outline' className='rounded-full border-slate-200 bg-slate-50 text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300'>
      ...
      <CardContent className='space-y-5 p-5'>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isComplete && "border-emerald-200 bg-emerald-50/70 dark:border-emerald-900/70 dark:bg-emerald-950/30",
                  isCurrent && "border-sky-200 bg-sky-50/70 dark:border-cyan-900/70 dark:bg-cyan-950/30",
                  !isComplete && !isCurrent && "border-slate-200 bg-slate-50/70 dark:border-slate-800 dark:bg-slate-900",
                )}
              >
                <div
                  className={cn(
                    ...
                    !isComplete && !isCurrent && "bg-white text-slate-500 dark:bg-slate-800 dark:text-slate-400",
                  )}
                >
        ...
        <div className='grid gap-3 sm:grid-cols-2'>
          {tracking.estimatedCompletion ? (
            <div className='flex items-center gap-2 rounded-[22px] border border-slate-200 bg-slate-50/70 px-4 py-3 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300'>
          ...
          {tracking.labOrderId ? (
            <div className='rounded-[22px] border border-slate-200 bg-slate-50/70 px-4 py-3 dark:border-slate-800 dark:bg-slate-900'>
        ...
        {tracking.labLocation ? (
          <div className='rounded-[22px] border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900'>
