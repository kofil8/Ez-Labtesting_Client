...
export function AdminDashboard() {
  ...
  const { stats, revenueByMonth, ordersByStatus, topSellingTests, revenueByPaymentMethod, recentOrders } = useMemo(() => {
    ...
    const pendingResultsCount = results.filter((r: any) => r.status === 'pending' || r.status === 'processing').length
    ...
    const revenueGrowth = previousMonthRevenue > 0 
      ? ((revenueStats.totalRevenue - previousMonthRevenue) / previousMonthRevenue) * 100 
      : 0
                  ...
                  <tr key={order.id} className="border-b hover:bg-muted/50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {order.tests.slice(0, 2).map((t: any) => t.testName).join(', ')}
                    ...
                    <td className="py-3 px-4">
                      <span
                        className={cn(
                          ...
                          !['completed', 'pending', 'processing', 'cancelled'].includes(order.status) && 'bg-gray-100 text-gray-800'
                        )}
                      >
