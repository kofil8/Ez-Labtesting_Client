...
export function normalizeNotificationItem(value: unknown): NotificationItem {
  ...
  return {
    id:
      String(item.id ?? item._id ?? item.notificationId ?? "").trim() ||
...
export async function markNotificationRead(id: string): Promise<void> {
  ...
  const response = await clientFetch(
    getApiUrl(`/notifications/${encodedId}/read`),
    {
      method: "PATCH",
  ...
  await assertOk(response, "Failed to mark notification as read");
}
...
export async function markAllNotificationsRead(): Promise<void> {
  ...
  await assertOk(response, "Failed to mark all notifications as read");
}

export async function deleteNotification(id: string): Promise<void> {
  const encodedId = encodeURIComponent(id);
  const response = await clientFetch(getApiUrl(`/notifications/${encodedId}`), {
    method: "DELETE",
    headers: buildJsonHeaders(),
  });

  await assertOk(response, "Failed to delete notification");
}

export async function deleteAllNotifications(): Promise<void> {
  const response = await clientFetch(getApiUrl("/notifications"), {
    method: "DELETE",
    headers: buildJsonHeaders(),
  });

  await assertOk(response, "Failed to delete notifications");
}
...
export class NotificationsApiService {
  ...
  static deleteAllNotifications = deleteAllNotifications;
