...
export function normalizePublicTest(rawTest: any): PublicCatalogTest {
  ...
  return {
    ...
    isPhysicianReviewed:
      typeof rawTest?.isPhysicianReviewed === "boolean"
        ? rawTest.isPhysicianReviewed
        : undefined,
    isPanel:
      typeof rawTest?.isPanel === "boolean" ? rawTest.isPanel : undefined,
    totalOrders:
      normalizeNumber(rawTest?.totalOrders) ??
      normalizeNumber(rawTest?.orderCount) ??
      normalizeNumber(rawTest?.soldCount) ??
      undefined,
    soldCount:
      normalizeNumber(rawTest?.soldCount) ??
      normalizeNumber(rawTest?.totalOrders) ??
