...
export async function verifyResetOtp(email: string, otp: string) {
  ...
  const payload = await requestAuthEndpoint<{ resetToken: string }>(
  ...
  return { success: true, resetToken: payload.data?.resetToken };
}
...
export async function resetPassword(input: ResetPasswordInput) {
  ...
  if (
    !("resetToken" in payload
      ? payload.resetToken
      : payload.email && payload.otp) ||
    !payload.newPassword
  ) {
    throw new Error("Reset token or email/OTP and new password are required");
  }
...
export async function verifySensitiveMFA(
  token: string,
