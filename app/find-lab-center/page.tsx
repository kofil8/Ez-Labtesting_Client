...
import {
  fetchMedicalLabsNear,
  geocodeLocation,
  mapboxFeatureToLabCenter,
} from "@/lib/mapbox-search";
import { LabCenter } from "@/types/lab-center";
import { motion } from "framer-motion";
import { List, Map, TestTube2 } from "lucide-react";
...
export default function FindLabCenterPage() {
  ...
  // Fetch medical labs near user location
  const fetchLabsNearLocation = async (lat: number, lng: number) => {
    setIsLoadingLabs(true);
    try {
      const features = await fetchMedicalLabsNear(lat, lng, 50);
      const labs = features.map(mapboxFeatureToLabCenter);
      setLabCenters(labs);
      setFilteredLabCenters(labs);
    } catch (error) {
      console.error("Error fetching labs:", error);
      toast({
        title: "Error",
        description: "Failed to load medical labs. Please try again.",
        variant: "destructive",
      });
    } finally {
  ...
  // Auto-request user location on page load - only show labs if granted
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          // Fetch medical labs near user location
          await fetchLabsNearLocation(latitude, longitude);

          toast({
            title: "Location found",
            description: "Showing medical labs near you.",
          });
        },
        (error) => {
          console.log("User declined location or error:", error);
          // Don't show any labs if location is denied
          setLabCenters([]);
          setFilteredLabCenters([]);
        },
        { timeout: 10000 }
      );
    }
  ...
  const handleSearch = async (
    ...
    if (!coords && location.trim()) {
      ...
      if (!geocoded) {
        toast({
          ...
          description:
            "Could not find the specified location. Please try a different search.",
          variant: "destructive",
        });
        return;
      }
    ...
    if (coords) {
      ...
      // Fetch medical labs near searched location
      await fetchLabsNearLocation(coords.lat, coords.lng);
    }
  };
  ...
  const handleRadiusChange = async (radius: number) => {
    ...
    if (userLocation) {
      // Re-fetch labs with new radius
      await fetchLabsNearLocation(userLocation.lat, userLocation.lng);
    }
  };
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          toast({
            ...
            description: "Please enable location access to find nearby labs.",
            variant: "destructive",
          });
        }
      );
        ...
        >
          <div className='text-center max-w-md'>
            ...
            <p className='text-gray-600 mb-4'>
              Please add your Mapbox access token to .env.local to use this
              feature.
            </p>
          ...
          <div className='container mx-auto px-4 sm:px-6 lg:px-8'>
            ...
            >
              ...
              <p className='text-base sm:text-lg md:text-xl text-blue-100 max-w-2xl mx-auto px-4'>
                Locate convenient lab testing centers near you. Get directions,
          ...
          <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4'>
            <div className='w-full sm:w-auto'>
              <h2 className='text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100'>
                {filteredLabCenters.length} Lab Center
                {filteredLabCenters.length !== 1 ? "s" : ""} Found
              </h2>
            ...
            <div className='max-w-4xl mx-auto'>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6'>
                ...
                >
                  ...
                  <h3 className='font-bold text-lg sm:text-xl mb-2 sm:mb-3 text-center text-gray-900 dark:text-gray-100'>
                    Search Location
                  </h3>
                  <p className='text-xs sm:text-sm text-gray-600 dark:text-gray-400 text-center leading-relaxed'>
                    Enter your city, ZIP code, or use your current location to
                    find nearby labs within your preferred radius.
                  </p>
                </motion.div>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  ...
                  <p className='text-xs sm:text-sm text-gray-600 dark:text-gray-400 text-center leading-relaxed'>
                    Compare ratings, hours, distances, and locations to find the
                    most convenient lab center for your needs.
                  </p>
                </motion.div>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  ...
                  <p className='text-xs sm:text-sm text-gray-600 dark:text-gray-400 text-center leading-relaxed'>
                    Visit the lab with your order confirmation to complete your
                    testing. Get directions with one click!
                  </p>
                </motion.div>
              </div>
            </div>
